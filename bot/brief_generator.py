...
async def run(target_telegram_id: Optional[int] = None) -> None:
    """
    Full pipeline:
      1. Prune stale seen_articles
      2. Fetch articles from NewsAPI + RSS
    ...
    if not all_articles:
        logger.warning("No articles fetched — aborting brief run")
        return
...
if __name__ == "__main__":
    ...
    asyncio.run(run(target_telegram_id=args.user))
